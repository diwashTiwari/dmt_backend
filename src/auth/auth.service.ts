...
export class AuthService {
  ...
  async resendEmailConfirmation(email: string) {
    try {
      ...
      if (!user) {
        return Promise.reject(new BadRequestException('User not found. Cannot send verify email!'));
      }

      if (user.isEmailConfirmed) {
        return Promise.reject(new BadRequestException('Email already confirmed'));
      }
  ...
  async emailConfirmation({ token }: ConfirmEmailDto) {
    try {
      ...
      if (typeof payload === 'object' && 'email' in payload) {
        ...
        if (!user) {
          return Promise.reject(new UnprocessableEntityException('User not found. Cannot confirm email!'));
        }
        ...
        return Promise.reject(new BadRequestException('Token verification has been failed!'));
      }
      ...
      if (e?.name === 'TokenExpiredError') {
        return Promise.reject(new BadRequestException('Email confirmation token expired'));
      }
  ...
  async forgotPassword(email: string) {
    try {
      ...
      if (!user) {
        return Promise.reject(new BadRequestException('User not found. Cannot send reset password email!'));
      }
  ...
  async resetPassword(token: string, password: string) {
    try {
      ...
      if (typeof payload === 'object' && 'email' in payload) {
        ...
        if (!user) {
          return Promise.reject(new BadRequestException('Invalid token!. User not found. Cannot reset password!'));
        }
      ...
      if (e?.name === 'TokenExpiredError') {
        return Promise.reject(new BadRequestException('Password reset token expired'));
      }
  ...
  async login(loginDto: LoginDto) {
    ...
    if (!user.isEmailConfirmed) {
      throw new UnauthorizedException('Unauthorized: Please verify your email!');
    }


    const isValidPassword = await bcrypt.compare(loginDto.password, user.password);
    if (!isValidPassword) throw new UnauthorizedException('Invalid password, please try again.');
